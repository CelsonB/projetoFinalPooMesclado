...
public class AgendaDAO {
	...
	public void cadastrarAgenda(Agenda agenda, int idUsuario) throws SQLException {
		...
		try {
			st = conn.prepareStatement("insert into agenda(nome, descricao, usuario_id) values(?, ?, ?)");
	...
	public List<Agenda> buscarAgendas(int idUsuario) throws SQLException {
		...
		try {
			...
			st = conn.prepareStatement("select * from agenda where usuario_id = ? order by nome");
